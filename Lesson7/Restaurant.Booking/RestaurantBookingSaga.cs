...
namespace Restaurant.Booking
{
	public sealed class RestaurantBookingSaga : MassTransitStateMachine<RestaurantBooking>
	{
		...
		public RestaurantBookingSaga(ILogger<RestaurantBookingSaga> logger)
		{
			...
			Event(() => BookingCancellationRequest, x => x.CorrelateById(context => context.Message.OrderId));
			...
			Initially(
				When(BookingRequested)
					.Then(context =>
					{
						...
						_logger.LogInformation("Saga: {SagaCreationTime}", context.Data.CreationDate);
					})
					.Schedule(BookingExpired,
						context => new BookingExpire(context.Instance),
						context => TimeSpan.FromSeconds(20))
			...
			During(AwaitingBookingApproved,
				...
				When(BookingRequestFault)
					.Then(context => _logger.LogWarning("Ошибочка вышла!"))
				...
				When(BookingExpired.Received)
					.Then(context => _logger.LogWarning("Отмена заказа {OrderId}", context.Instance.OrderId))
					.Publish(context => (IBookingCancellation)
			...
			During(AwaitingGuestArrival,
				...
				When(GuestWaitingExpired.Received)
					.Then(context => _logger.LogWarning("Гость вовремя не прибыл {OrderId}", context.Instance.OrderId))
					.Publish(context => (IBookingCancellation)
						new BookingCancellation(context.Instance.OrderId))
					.Finalize()
			);
