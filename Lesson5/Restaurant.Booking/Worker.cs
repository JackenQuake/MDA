			...
			while (!stoppingToken.IsCancellationRequested)
			{
				...
				switch (choice)
				{
					case 1:
						...
						Dish PreOrder = (Dish) rnd.Next(0, 4); // Выбираем случайный тип предзаказа, четверть всех заказов будет с лазаньей
						await _bus.Publish((IBookingRequest)new BookingRequest(NewId.NextGuid(), NewId.NextGuid(), PreOrder, DateTime.Now), stoppingToken);
						break;
					case 2:
